...
async function collect(iD: Data): Promise<Data> {
  ...
  if (iD.dirs.internalResources.isFound) {
    ...
    iD.assets.miscAssets.push(
      ...(await getIfFilesExist(path.internalResourcesAssets))
    )
  }
  ...
  if (iD.dirs.canvasLandingPages.isFound) {
    ...
    iD.module.meta.didContainFallbackClp = canvasLandingPagesPaths.some(
  ...
  if (iD.dirs.levelUp.isFound) {
    ...
    // file.
    const levelUpDirOnlyHoldsReadmeFile =
      levelUpFilesPaths.length <= 1 && levelUpFilesPaths.includes("README.md")
