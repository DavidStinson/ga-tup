...
import { 
  checkNeedToMakeLvlUpMlDirs, getLvlUpMlDirData
} from "../dir/lvl-up.js"
...
async function getData(iD: Data): Promise<Files> {
  ...
  iD.files.invalidLvlUpFiles = lvlUpFilePaths.filter((filepath) => (
    path.extname(filepath) !== ".md"
  ))

  const lvlUpFileNames = validLvlUpMlFilePaths.map((lvlUpPath) => (
    path.parse(lvlUpPath).name
  ))
  
  iD.files.lvlUpMls = await getMlFilesData(
    lvlUpFileNames, validLvlUpMlFilePaths, "lvl-up-ml"
  )
...
function checkLvlUpMlMove(iD: Data): Files {
  iD.files.lvlUpMls = iD.files.lvlUpMls.map((ml) => {
    const lvlUpDir = iD.dirs.lvlUpMls.find((dir) => (
      dir.dirName === ml.fileName
    ))
    return {
      ...ml,
