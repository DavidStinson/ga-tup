...
function process(iD: Data): ResultMsgs {
  ...
  iD.resultMsgs = processTemplateFile(
    iD.resultMsgs, files.defaultLayout, verbose
  )

  iD.resultMsgs = processTemplateFileWithHeading(
    iD.resultMsgs, files.rootReadme, verbose
  )

  iD.resultMsgs = processTemplateDir(
    iD.resultMsgs, dirs.internalResources, verbose
  )
  iD.resultMsgs = processTemplateDir(
  ...
  iD.resultMsgs = processTemplateFileWithHeading(
    iD.resultMsgs, files.videoHub, verbose
  )
  iD.resultMsgs = processTemplateFileWithHeading(
    iD.resultMsgs, files.releaseNotes, verbose
  )
  iD.resultMsgs = processTemplateFileWithHeading(
    iD.resultMsgs, files.instructorGuide, verbose
  )
  ...
  if (iD.module.meta.type === "lecture") {
    ...
    iD.resultMsgs = processTemplateFileWithHeading(
      iD.resultMsgs, files.references, verbose
    )
    iD.resultMsgs = processLevelUpDir(
      iD.resultMsgs, dirs.lvlUp, iD.module, verbose
    )
  }
...
function processConfigJson(msgs: ResultMsgs, meta: Meta): ResultMsgs {

  return msgs
}

function processLevelUpDir(
  msgs: ResultMsgs, dir: TemplateDir, module: Module, verbose: boolean
): ResultMsgs {
