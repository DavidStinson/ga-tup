...
// do the thing
async function verifyData(iD: Data): Promise<Data> {
...
async function moduleTypeCollect(): Promise<"lectureTemplateUrl" | "labTemplateUrl"> {
  try {
    return await select({
...
async function verifyAndCollectMlTitles(mls: Dir[]): Promise<Dir[]> {
  return (await verifyMlTitles(mls)) ? mls : await collectMlTitles(mls)
}

async function verifyMlTitles(mls: Dir[]): Promise<boolean> {
...
async function collectMlTitles(mls: Dir[]): Promise<Dir[]> {
  ...
  try {
    while(true) {
      ...
      const selectedMlIdx = mls.findIndex((ml) => (
        ml.dirName === editingMl.dirName
      ))
