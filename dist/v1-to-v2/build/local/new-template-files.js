// local
import { buildSubHeading, removeHero, updatePrefixAndHeadline, } from "./helpers.js";
// do the thing
function buildNewTemplateFiles(iD) {
...
function buildNewDefaultLayout(iD) {
    const moduleTitle = iD.module.prefix
        ? `${iD.module.prefix} - ${iD.module.headline}`
        : iD.module.headline;
    iD.files.defaultLayout.newFile = iD.files.defaultLayout.templateFile.replace("<title>[tktk Module Name]</title>", `<title>${moduleTitle}</title>`);
