...
function checkCanvasLandingPages(msgs, files, fallbackPage) {
    if (!files.length) {
    ...
    if (!fallbackPage) {
        msgs.failures.push("No fallback canvas landing page was found - it will be created, but manual migration will be required.");
    }
    files.forEach((file) => {
...
function checkMicrolessons(msgs, files, isLevelUp) {
    if (files.length === 1) {
        msgs.successes.push(`There is 1${isLevelUp ? " level up " : " "}microlesson.`);
    }
    else if (files.length > 1) {
        msgs.successes.push(`There are ${files.length}${isLevelUp ? " level up " : " "}microlessons.`);
    }
    else {
        if (isLevelUp) {
