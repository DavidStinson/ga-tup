// local
import { processGoodItem, processBadItem, processFileWithHeading } from "./helpers.js";
// do the thing
function process(iD) {
    ...
    if (iD.files.rootReadme.isFound) {
        iD.repoMsgs = processFileWithHeading(iD.repoMsgs, iD.files.rootReadme, verbose);
    }
    ...
    if (iD.files.releaseNotes.isFound) {
        iD.repoMsgs = processFileWithHeading(iD.repoMsgs, iD.files.releaseNotes, verbose);
    }
    iD.repoMsgs = processGoodItem(iD.repoMsgs, iD.files.instructorGuide);
    if (iD.files.instructorGuide.isFound) {
        iD.repoMsgs = processFileWithHeading(iD.repoMsgs, iD.files.instructorGuide, verbose);
    }
    ...
    if (iD.module.meta.type === "lecture") {
        ...
        if (iD.files.references.isFound) {
            iD.repoMsgs = processFileWithHeading(iD.repoMsgs, iD.files.references, verbose);
        }
...
function processRootDirForAssets(msgs, repo) {
