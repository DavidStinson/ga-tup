...
async function getData(iD) {
    ...
    iD.files.invalidLvlUpFiles = lvlUpFilePaths.filter((filepath) => (path.extname(filepath) !== ".md"));
    const lvlUpFileNames = validLvlUpMlFilePaths.map((lvlUpPath) => (path.parse(lvlUpPath).name));
    iD.files.lvlUpMls = await getMlFilesData(lvlUpFileNames, validLvlUpMlFilePaths, "lvl-up-ml");
...
async function getLvlUpFilePaths(iD) {
