...
async function getData(iD) {
    const clpWillExist = iD.dirs.clps.isFound ||
    ...
    const canvasLandingPageNames = canvasLandingPagesPaths.map((clpPath) => path.parse(clpPath).name);
    iD.files.clps = await getClpFilesData(canvasLandingPageNames, canvasLandingPagesPaths);
    return iD.files;
}
async function getClpFilesData(itemNames, filepaths) {
